...
describe("Cursor skills", () => {
  ...
  it("retains healthy skills and reports filesystem failures", async () =>
    await runNode(
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        ...
        yield* fileSystem.makeDirectory(path.join(workspace, ".cursor"));
        yield* fileSystem.writeFileString(path.join(workspace, ".cursor", "skills"), "blocked");
        const healthySkillDirectory = path.join(workspace, ".agents", "skills", "healthy");
        yield* fileSystem.makeDirectory(healthySkillDirectory, { recursive: true });
        yield* fileSystem.writeFileString(
          path.join(healthySkillDirectory, "SKILL.md"),
          "# Healthy\n",
        );

        const inspection = yield* inspectCursorSkills(workspace, { HOME: userHome });
        expect(inspection.skills.map((skill) => skill.name)).toEqual(["healthy"]);
        expect(inspection.errors[0]?._tag).toBe("PlatformError");
      }),
    ));
});
